...
https://doi.org/10.23199/jdqs.2026.8.1.001
...
본 연구의 핵심 제안인 메타학습 기반 이상탐지 모델은 두 데이터셋 모두에서 우수한 탐지 성능을 보였습니다.

### 8.1 메타모델 성능

| Dataset | AUC | Precision | Recall | Specificity | F1-score |
|---------|-----|-----------|--------|-------------|----------|
| NSL-KDD | 0.98 | 0.96 | 0.89 | 0.95 | 0.93 |
| UNSW-NB15 | 0.95 | 0.77 | 0.97 | 0.65 | 0.86 |

NSL-KDD에서는 정밀도, 재현율, 특이도, F1-score가 전반적으로 고르게 나타나 메타모델이 공격 탐지와 정상 판정을 비교적 균형 있게 수행함을 확인할 수 있었습니다.

UNSW-NB15에서는 재현율이 0.97로 매우 높게 나타나 다양한 공격 유형을 효과적으로 탐지하는 경향을 보였습니다. 다만 특이도는 0.65로 상대적으로 낮아 일부 정상 트래픽이 공격으로 판정되는 경향이 관찰되었습니다. 그럼에도 불구하고 AUC 0.95, F1-score 0.86으로 전체적인 탐지 성능은 높은 수준을 유지하였습니다.

### 8.2 해석

이 결과는 서로 다른 비지도 이상탐지 모델이 생성한 이상점수를 통합함으로써, 개별 모델의 한계를 상호 보완하고 더 안정적인 탐지 성능을 확보할 수 있음을 시사합니다.
